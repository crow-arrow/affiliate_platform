...
export const Dashboard = () => {
  ...
  const lastThreeTrips = trips?.slice().sort((a,b) => new Date(b.booking_date) - new Date(a.booking_date)).slice(0, 3).reverse()
    ...
    <div className="w-full space-y-6">
      <div className="grid grid-cols-4 md:grid-cols-6 lg:grid-cols-3 flex-wrap gap-4 2xl:w-[1140px]">
          ...
          <div className="flex flex-col gap-4">
            <div className="grid grid-cols-4 max-[600px]:grid-cols-2 gap-3 xl:gap-6 w-full flex-grow">
              ...
              <div className="flex flex-col w-full bg-primaryLite dark:bg-primary p-2 xl:p-4 rounded-xl text-gray-800 dark:text-gray-100">
                ...
                <b className="lg:text-xl xl:text-2xl">
                  <AnimatedNumber value={totalRevenue || 0} suffix=" €" decimals={0} />
                </b>
              ...
              <div className="flex flex-col w-full bg-primaryLite dark:bg-primary p-2 xl:p-4 rounded-xl text-gray-800 dark:text-gray-100">
                ...
                <b className="lg:text-xl xl:text-2xl">
                  <AnimatedNumber value={commission || 0} suffix=" €" decimals={0} />
                </b>
              ...
              <table className="w-full text-xs text-left text-gray-700 dark:text-gray-200 mt-2">
                <thead className="text-xs text-gray-600 dark:text-gray-300">
                  <tr>
                    ...
                    <th scope="col" className="px-4 py-2">Booking Date</th>
                ...
                <tbody>
                  {lastThreeTrips?.map((trip) => (
                    <tr key={trip.id} className="border-b border-gray-200 dark:border-gray-700">
                      <td className="px-4 py-2">{trip.id}</td>
                      <td className="px-4 py-2">{new Date(trip.booking_date).toLocaleDateString('en-US', {
                        month: 'short',
                        day: 'numeric',
                        year: 'numeric',
                      })}</td>
                      <td className="px-4 py-2">{parseFloat(trip.total_price).toFixed(2)}€</td>
                      <td className="px-4 py-2">
                        <span
                          className={`
                            rounded-lg px-4 py-0.5 capitalize text-center justify-start
                            ${trip.order_status === 'cancel' ? 'bg-accentPink/30 text-accentPink' 
                            : trip.order_status === 'rejected' ? 'bg-gray-300/30 text-gray-300' 
                            : trip.order_status === 'pending' ? 'bg-accentOrange/30 text-accentOrange' 
                            : trip.order_status === 'wait-for-approval' ? 'bg-accentBlue/30 text-accentBlue' 
        ...
        <div className="flex flex-col max-[600px]:col-span-4 max-[600px]:-order-1 max-md:order-2 col-span-2 lg:col-span-1 gap-1 w-full h-full p-4 rounded-2xl bg-white dark:bg-secondary">
          ...
          <div>
            <h3 className="text-md font-semibold text-gray-400">Contact Email:</h3>
            <a href="mailto:nadine@jinn-travel.com" className="hover:text-accent transition-colors">
              nadine@jinn-travel.com
            </a>
          </div>
        </div>
        <div 
          className="flex justify-between relative flex-col max-[600px]:col-span-4 max-[600px]:-order-2 col-span-2 lg:col-span-1 w-full h-full p-4 xl:p-8 rounded-2xl bg-white dark:bg-secondary"
        >
          <dl className="flex justify-between">
            <dt className="font-bold text-xl">{userLevel}</dt>
            <dd className="text-sm">{userName} - ID: {currentUser.id}</dd>
          </dl>
          <ProgressBar />
          <span className="text-sm px-4 py-2 my-2 rounded-xl border border-gray-400">Next level: {(userLevel === "Gold" || userLevel === "Silver") ? "Gold" : userLevel === "Bronze" && "Silver"}</span>
        </div>
