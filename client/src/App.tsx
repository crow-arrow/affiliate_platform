...
function App() {
  ...
  useEffect(() => {
    ...
    // Проверяем возможность резолвить tenant из URL (path-based роутинг)
    const slugFromPath = extractTenantSlugFromPath(location.pathname);
    const slugFromQuery = searchParams.get("tenant") || searchParams.get("slug");

    const canResolveTenantFromUrl = slugFromPath || slugFromQuery;

    // Если нет способа резолвить tenant из URL и резолв завершен - 404
    if (
      !canResolveTenantFromUrl &&
      !isLoading &&
      (tenantStatus === "idle" || tenantStatus === "failed")
    ) {
      hasRedirectedRef.current = true;
