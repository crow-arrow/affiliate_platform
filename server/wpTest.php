...
if (!defined('AFFILIATE_TRACKING_PLUGIN_LOADED')) {
    ...
    add_action('wp_loaded', 'set_affiliate_cookie', 1);
    register_activation_hook(__FILE__, 'affiliate_create_tables');

    // Подключение файла с функциями (если он нужен для других целей)
    ...
    function affiliate_add_page_callback() {
        ...
        if (isset($_POST['add_affiliate'])) {
            ...
            $affiliates_table_name = $wpdb->prefix . 'affiliates';
            $wpdb->insert($affiliates_table_name, [
                'name' => $name,
                'email' => $email,
    ...
    // Устанавливаем куку для affiliateId и логируем клик
    function set_affiliate_cookie($affiliate_id = null) {
        error_log("✅ set_affiliate_cookie() вызвана");
    
        if ($affiliate_id === null && isset($_GET['affiliateId'])) {
            $affiliate_id = sanitize_text_field($_GET['affiliateId']);
            error_log("➡️ affiliateId из URL: $affiliate_id");
        }
        ...
        if ($affiliate_id) {
            ...
            if (!isset($_COOKIE[$cookie_name]) || $_COOKIE[$cookie_name] !== $affiliate_id) {
                $result = setcookie($cookie_name, $affiliate_id, time() + (86400 * 30), '/', $cookie_domain, is_ssl(), true);
                error_log("🍪 Кука установлена: $cookie_name = $affiliate_id, результат: " . ($result ? 'успех' : 'ошибка'));
                
                log_referral_click($affiliate_id);
            } else {
                error_log("ℹ️ Кука уже установлена ранее, пропускаем");
            }
        } else {
            error_log("⚠️ affiliateId не найден, клик не логируется");
        }
    }
